...
# tf.function decorator to avoid re-tracing issue; in fact, no
#   no `tf.function` introduces the re-tracing issue, just as abusing does
# it should only be used for the training step function;
# the model is fine, check the anchor functions
@tf.function
def train_rpn_step(
    mdl: tf.keras.Model,
    images: tf.Tensor,
